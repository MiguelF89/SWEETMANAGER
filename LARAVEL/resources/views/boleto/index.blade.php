        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600">
                    Gerencie seus boletos a pagar e pagos.
                </p>
            </div>

            <div class="flex flex-wrap gap-2">
                <a href="{{ route('boleto.reader') }}"
                   class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">
                    Ler novo boleto
                </a>

                <a href="{{ route('relatorio.index') }}"
                   class="inline-flex items-center px-4 py-2 bg-slate-600 text-white rounded-md hover:bg-slate-700 transition">
                    Relatório
                </a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                ...
                <div class="bg-white shadow-sm sm:rounded-lg p-6">
                    ...
                    <p class="mt-2 text-2xl font-semibold text-blue-700">
                        R$ {{ number_format($totais['total'], 2, ',', '.') }}
                    </p>
                </div>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('boleto.index') }}"
                   ...
                   {{ $status === 'todos' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}">
                    Todos
                </a>

                <a href="{{ route('boleto.index', ['status' => 'pendente']) }}"
                   class="px-3 py-2 rounded-md text-sm font-medium transition
                   {{ $status === 'pendente' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}">
                    Pendentes
                </a>

                <a href="{{ route('boleto.index', ['status' => 'pago']) }}"
                   class="px-3 py-2 rounded-md text-sm font-medium transition
                   {{ $status === 'pago' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }}">
                    Pagos
                </a>
